        ...
        case 0: //Login State
        {
            ...
            gm.mysql.handle.query("SELECT * FROM banip WHERE username = ? OR PlayerIP = ?", [username, player.ip], function (e, res3) {
                if (res3[0]) { // Ban
                    ...
                    gm.mysql.handle.query('SELECT `password` FROM `accounts` WHERE `username` = ? OR `SocialClub`', [username, player.socialClub], function(err, res){
                    // gm.mysql.handle.query('SELECT * FROM `accounts` WHERE `SocialClub` = ?', [player.socialClub], function (err, res4) {
                        if(res.length > 0 || res.socialClub > 0){
                            // if (res4.length > 0) {
                            ...
                            // } else {
                            //     layer.call("loginHandler", ["incorrectinfo2"]);
                            // }
                        } else {
                            player.call("loginHandler", ["incorrectinfo"]);
                        }
        ...
        case 1: //Register State
        {
            gm.mysql.handle.query('SELECT * FROM accounts WHERE username = ?', [username], function (err, res) {
                if (res.length > 0) {
                    ...
                    gm.mysql.handle.query('SELECT * FROM accounts WHERE Mail = ?', [Mail], function (err, res4) {
                        if (res4.length > 0) {
                        player.call("loginHandler", ["taken2info"]);
                    } else {
                        gm.mysql.handle.query('SELECT * FROM accounts WHERE SocialClub = ?', [player.socialClub], function (err, res5) {
                            if (res5.length > 0) {
                                    ...
                                    if ((password.length >= 6)) {
                                        ...
                                        if (username.length <= 16) {
                                            ...
                                            if (password.length <= 24) {                                          
                                                ...
                                                if (!Mail.length == 0) {        
                                                    if (!(Mail.length > 0 && (Mail.match(/.+?\@.+/g) || []).length !== 1)) {
                                                        ...
                                                        player.call("loginHandler", ["email"]);
                                                    }
                                                } else {
                                                    player.call("loginHandler", ["m"]);
                                                }
                                            } else {
